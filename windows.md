...
`从不通知`
    ...
    * 安装`Neat Download Manager` [[neatdownloadmanager.com]](http://www.neatdownloadmanager.com/index.php/en/)
      * `Settings`
        * `General`
          * `Max Connections per Download` `32`
          * `Get File Creation Date From Server`
        * `Proxy/Socks`  
          * `HTTP Proxy`  
`Address` `localhost`  
`Port` `7897`
...
`关`
  ...
  * 驱动
    * 安装`NVIDIA` [[nvidia.cn]](https://www.nvidia.cn/software/nvidia-app/)
      * `NVIDIA`
        * `图形`  
`全局设置`
          * `低延时模式` `超高`
...
`全局设置`  
    ...
    * `DDU` [[wagnardsoft.com]](https://www.wagnardsoft.com/display-driver-uninstaller-DDU)
      * 安装/更新驱动
    * 安装`英特尔驱动程序和支持助理` [[intel.cn]](https://www.intel.cn/content/www/cn/zh/support/intel-driver-support-assistant.html)
    * 安装`360驱动大师` [[dm.weishi.360.cn]](https://dm.weishi.360.cn/home.html)
    * 安装`Nvidia Broadcast` [[nvidia.cn]](https://www.nvidia.cn/geforce/broadcasting/broadcast-app/)
    * 安装`SteelSeries GG` [[cn.steelseries.com]](https://cn.steelseries.com/gg)
    * 下载`OpenRGB` [[openrgb.org]](https://openrgb.org/#downloads)
    * 安装`FL Esports` [[flesports.com]](http://www.flesports.com/down/)
  * 设置
    * `系统`
      * `屏幕`  
`显示卡`  
